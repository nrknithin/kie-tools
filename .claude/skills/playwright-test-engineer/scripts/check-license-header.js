...
const { resolveRepoRoot, groupForFile, headerFilePathForGroup, UNCONFIGURED_HEADER_SENTINEL } = require("./lib/workspace");
...
function main() {
  ...
  for (const file of files) {
    ...
    if (!content.startsWith(expected)) {
      problems.push({ file, group, issue: `missing or non-verbatim header at top of file (expected ${path.relative(repoRoot, headerPath)})` });
    }
  }
