...
const { resolveRepoRoot, listWorkspacePackages, walkFiles, groupForFile, headerFilePathForGroup, UNCONFIGURED_HEADER_SENTINEL } = require("./lib/workspace");
...
function checkHeader(repoRoot, file, source) {
  ...
  if (!fs.existsSync(headerPath)) return { severity: "warning", rule: "license-header", message: `no header file found for group "${group}"` };
  const expected = fs.readFileSync(headerPath, "utf8").replace(/\r\n/g, "\n").trimEnd();
  if (expected === UNCONFIGURED_HEADER_SENTINEL) {
    return { severity: "warning", rule: "license-header", message: `header for group "${group}" not configured yet (see assets/.ibm-header) — skipped` };
  }
  if (!source.replace(/\r\n/g, "\n").startsWith(expected)) {
    return { severity: "high", rule: "license-header", message: "missing or non-verbatim license header for this file's group" };
  }
  return null;
}
...
function checkFile(repoRoot, file) {
  ...
  if (!/^[a-z][a-zA-Z0-9]*\.spec\.ts$/.test(basename)) {
    findings.push({ severity: "high", rule: "naming", message: `"${basename}" should be camelCase.spec.ts naming the action (e.g. addInputData.spec.ts)` });
  }
  ...
  if (!/__fixtures__\/base["']/.test(source)) {
    findings.push({ severity: "high", rule: "fixtures-import", message: 'no `import { test, expect } from ".../__fixtures__/base"` found — spec may bypass the shared fixture composition' });
  }

  for (const m of findAll(source, /page\.waitForTimeout\(/g)) {
    findings.push({ severity: "high", rule: "hardcoded-wait", line: lineOf(source, m.index), message: "page.waitForTimeout(...) — wait on a locator/condition instead" });
  }

  for (const m of findAll(source, /\bpage\.locator\(/g)) {
    findings.push({ severity: "medium", rule: "raw-selector", line: lineOf(source, m.index), message: "page.locator(...) used directly in a spec — prefer a page-object fixture method" });
  }

  for (const m of findAll(source, /toHaveScreenshot\(\s*["']([^"']+)["']/g)) {
    const name = m[1];
    if (!/^[a-z0-9]+(-[a-z0-9]+)*\.png$/.test(name)) {
      findings.push({ severity: "medium", rule: "screenshot-naming", line: lineOf(source, m.index), message: `"${name}" is not kebab-case-with-.png` });
    }
  }
  ...
  for (let i = 0; i < testStarts.length; i++) {
    ...
    if (expectCalls.length === 1 && /expect\([^)]*\)\.toHaveScreenshot/.test(chunk)) {
      findings.push({ severity: "low", rule: "screenshot-only", line: lineOf(source, start), message: "test's only assertion is a screenshot — consider adding a functional/model assertion too" });
    }
  }
...
function main() {
  ...
  const results = specFiles.map((f) => checkFile(repoRoot, f)).filter((r) => r.findings.length > 0);
  ...
  console.log(JSON.stringify({ pkgDir: path.relative(repoRoot, pkgDir), specFilesChecked: specFiles.length, bySeverity, findings: flat }, null, 2));

  if (bySeverity.high > 0) {
    console.error(`\n${bySeverity.high} high-severity finding(s). Fix before presenting work as done.`);
    process.exit(1);
  }
  console.error(`\nNo high-severity findings across ${specFiles.length} spec file(s). ${bySeverity.medium + bySeverity.low} lower-severity note(s) reported above.`);
}
